...
class VideoAdapter(nn.Module):
    def __init__(
        ...
        # 【AMAL】初始化 Sigma
        self.task_sigma = None

        self.init()

    # 【AMAL】接收 Sigma
    def set_scale(self, sigma):
        self.task_sigma = sigma
    ...
    def forward(self, x: Tensor, casual_mask, **kwargs):
        ...
        if self.modal_dynamic and self.md_pos in ['down in cls', 'down out cls']:
            ...
            # 【核心修改: Residual Gating】
            if self.task_sigma is not None:
                # 初始 task_sigma=0，意味着 scale=1 (无变化)
                # 这保证了 Epoch 1 的性能不会崩塌
                down = down * (1.0 + self.task_sigma)
